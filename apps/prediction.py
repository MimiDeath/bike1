...
def app(df: pd.DataFrame, current_dir: Path):
   ...
   """)
    ...
    st.markdown("""
        ...
        Для линейной регрессии мы выбираем переменные, которые предположительно имеют линейную связь с зависимой переменной (в данном случае cnt).
    """)
    # Для упрощения выберем несколько предикторов
    predictors = df.columns.drop('cnt')
    target = 'cnt'
    ...
    with tab1:
        ...
        st.markdown("""
            **Описание:** Тренировочные данные используются для подгонки модели и оценки её параметров. 
            Эти данные получены путем исключения из исходного датасета столбцов с временными метками и целевой переменной 'cnt'.
        ...
        st.markdown("""
            **Целевая переменная (y_train)**.
            ...
            В качестве целевой переменной для тренировочного набора используются исключительно значения столбца 'cnt'.
        """)
